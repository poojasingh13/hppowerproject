<?php  include 'globalvar.php';?>
<!DOCTYPE html>
<html lang="en">
<head>
	<title>Thermal Power - MOEF</title>
    
	
<?php  include 'meta.html';?>    
</head><!--/head-->
<body data-spy="scroll" data-target="#navbar" data-offset="0">
<?php include 'header.html';?>
    <section id ="recognition">
		<div class="container">
            <div class="box">
                <div class="center gap">
                    <h2>MoEF EC Compliance Report</h2>
					<div class="lead">
<table width="974" border="0" cellspacing="0" cellpadding="0">
  
  <tr>
    <td align="left" class="h-bor"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="211" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><img src="images/spacer.gif" width="211" height="1" /></td>
          </tr>
          <tr>
            <td align="left"><!--#include file="inc-leftlinks-thermal.asp"--></td>
          </tr>
        </table></td>
        <td width="100%" valign="top" style="padding-left:15px;"><table border="0" width="95%" cellspacing="0" cellpadding="0" id="text">
          
          <tr>
            <td valign="top" align="left" width="33%"><table border="0" cellpadding="0" cellspacing="0" width="100%">
              <tr>
                <td valign="top" height="265" width="33%"><img src="images/emr-ec-sep12.gif" class="impadbor" /></td>
                <td valign="top" width="33%"><img src="images/emr-rec1-nov12.gif" class="impadbor" /></td>
                <td valign="top"><!--<img src="images/EC-crapr11.gif" class="impadbor" />--></td>
              </tr>
              <tr>
                <td valign="top" height="42"><b>EC Compliance Six monthly</b><br />
                  March, 2014
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" id="table9" style="margin-top:7px;">
                  <tr>
                    <td width="30"><img src="images/pdf.gif" width="25" height="26" /></td>
                    <td><a target="_blank" href="pdfs/EC-Compliance-Six-monthly.pdf"> Download in PDF format</a></td>
                  </tr>
                </table></td>
                <td valign="top" height="42"><b>EC Compliance six monthly receipt</b><br />
                  March, 2014
                
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" id="table11" style="margin-top:7px;">
                  <tr>
                    <td width="30"><img src="images/pdf.gif" width="25" height="26" /></td>
                    <td><a target="_blank" href="pdfs/EC-Compliance-six-monthly-receipt.pdf"> Download in PDF format</a></td>
                  </tr>
                </table></td>
                <td valign="top"><!--<b>EC Compliance Report</b><br />
                  06 April, 2011<table border="0" cellpadding="0" cellspacing="0" width="100%" id="table13" style="margin-top:7px;">
                  <tr>
                    <td width="30"><img src="images/pdf.gif" width="25" height="26" /></td>
                    <td><a target="_blank" href="pdfs/EC-compliance-Report-apr11.pdf"> Download in PDF format</a></td>
                  </tr>
                </table>--></td>
              </tr>
              <tr>
                <td valign="top" width="33%">&nbsp;</td>
                <td width="33%" valign="top">&nbsp;</td>
                <td valign="top">&nbsp;
				</td>
              </tr>
              <tr>
                <td width="33%" valign="top"><!--<img src="images/EC-crJun12.gif" class="impadbor" />--></td>
                <td width="33%" valign="top">&nbsp;</td>
                <td valign="top">&nbsp;</td>
              </tr>
              <tr>
                <td valign="top" height="42"><!--<b>EC Compliance Report</b><br />
                  June, 2012<table border="0" cellpadding="0" cellspacing="0" width="100%" id="table14" style="margin-top:7px;">
                  <tr>
                    <td width="30"><img src="images/pdf.gif" width="25" height="26" /></td>
                    <td><a target="_blank" href="pdfs/EC-compliance-Report-Jun12.pdf"> 
					Download in PDF format</a></td>
                  </tr>
                </table>--></td>
                <td valign="top" height="42">&nbsp;</td>
                <td valign="top">&nbsp;</td>
              </tr>
              <tr>
                <td valign="top">&nbsp;
				</td>
                <td valign="top">&nbsp;
				</td>
                <td valign="top">&nbsp;</td>
              </tr>
            </table>
          <p>&nbsp;</p>
                      <p>&nbsp;</p>
			<p>&nbsp;</p>
			<p>&nbsp;</p></td>
          </tr>
        </table></td>
      </tr>
    </table></td>
  </tr>
  <tr>
    <td align="left"><!--#include file="inc-footer.asp"--></td>
  </tr>
</table>

					</div>
					</div></div></div>
	</section>
    
	
<?php include 'footer.html';?>

    

